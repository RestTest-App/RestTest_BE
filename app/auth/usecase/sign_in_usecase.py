...
class SignInUseCase:
    ...
    async def execute(self, email: str, auth_provider: str):
        ...
        if not user:
            raise NotFoundException(message="등록된 소셜 계정이 아닙니다.")
        return user

    # async def execute(self, kakao_token: str):
    #     kakao_service = KakaoAuthService()
    #     info = await kakao_service.fetch_kakao_user_info(kakao_token)
    #     user = await AuthRepository.get_user_by_email_and_provider(
    #         self.db,
    ...
    # auth test용
    async def execute_test(self, kakao_token: str):
        kakao_service = KakaoAuthService()
        info = await kakao_service.fetch_user_into_test(kakao_token)
        user = await AuthRepository.get_user_by_email_and_provider(
            self.db,
            auth_provider=info["auth_provider"],
            email=info["email"]
